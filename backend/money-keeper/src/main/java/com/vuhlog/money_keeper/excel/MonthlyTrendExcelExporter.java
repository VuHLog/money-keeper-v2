...
import com.vuhlog.money_keeper.dto.response.responseinterface.report.ReportMonthlyTrend;
...
public class MonthlyTrendExcelExporter {
    private XSSFWorkbook workbook;
    private XSSFSheet sheet;
    private List<ReportMonthlyTrend> data;
    private ReportFilterOptionsRequest request;
    private List<String> timeArr = new ArrayList<>();
    private String transactionType = "";

    public MonthlyTrendExcelExporter(List<ReportMonthlyTrend> data) {
        this.data = data;
        workbook = new XSSFWorkbook();
        sheet = workbook.createSheet("Xu hướng chi tiêu theo danh mục");
        this.timeArr.addAll(Arrays.asList(ArrayUtils.toArray("Tháng 1", "Tháng 2", "Tháng 3", "Tháng 4", "Tháng 5", "Tháng 6", "Tháng 7", "Tháng 8", "Tháng 9", "Tháng 10", "Tháng 11", "Tháng 12")));
    ...
    private void setupPageProperties(ReportFilterOptionsRequest request) {
        ...
        header.setCenter("BÁO CÁO XU HƯỚNG " + this.transactionType.toUpperCase() + " THEO THÁNG");
    ...
    private void writeDataRows() {
        ...
        Map<String, ReportMonthlyTrend> dataMap = new HashMap<>();
        for (ReportMonthlyTrend report : data) {
            String time = report.getMonth();
        ...
        for (String time : timeArr) {
            ...
            ReportMonthlyTrend reportMonthlyTrend = dataMap.get(time);

            cell = row.createCell(1);
            cell.setCellValue(reportMonthlyTrend != null ? reportMonthlyTrend.getTotal() : 0);
    ...
    private void addSummaryRow(int rowIndex) {
        ...
        for (ReportMonthlyTrend monthlyTrend : data) {
            total += monthlyTrend.getTotal();
        }

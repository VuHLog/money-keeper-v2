...
import com.vuhlog.money_keeper.dto.response.responseinterface.report.ReportWeeklyTrend;
...
public class WeeklyTrendExcelExporter {
    private XSSFWorkbook workbook;
    private XSSFSheet sheet;
    private List<ReportWeeklyTrend> data;
    private ReportFilterOptionsRequest request;
    private List<String> timeArr = new ArrayList<>();
    private String transactionType = "";

    public WeeklyTrendExcelExporter(List<ReportWeeklyTrend> data) {
        this.data = data;
        workbook = new XSSFWorkbook();
        sheet = workbook.createSheet("Xu hướng chi tiêu theo danh mục");
        this.timeArr.addAll(Arrays.asList(ArrayUtils.toArray("Thứ 2", "Thứ 3", "Thứ 4", "Thứ 5", "Thứ 6", "Thứ 7", "Chủ nhật")));
    ...
    private void setupPageProperties(ReportFilterOptionsRequest request) {
        ...
        header.setCenter("BÁO CÁO XU HƯỚNG " + this.transactionType.toUpperCase() + " THEO TUẦN");
    ...
    private void writeDataRows() {
        ...
        Map<String, ReportWeeklyTrend> dataMap = new HashMap<>();
        for (ReportWeeklyTrend report : data) {
            String time = report.getDayOfWeek();
        ...
        for (String time : timeArr) {
            ...
            ReportWeeklyTrend reportWeeklyTrend = dataMap.get(time);

            cell = row.createCell(1);
            cell.setCellValue(reportWeeklyTrend != null ? reportWeeklyTrend.getTotal() : 0);
    ...
    private void addSummaryRow(int rowIndex) {
        ...
        for (ReportWeeklyTrend reportWeeklyTrend : data) {
            total += reportWeeklyTrend.getTotal();
        }
